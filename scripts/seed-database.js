...
async function main() {
	try {
		...
		for (const user of allUsers) {
			for (const race of allRaces.slice(0, 4)) {
				...
				if (!existingPrediction) {
					...
					// Randomly shuffle to create a unique prediction
		...
		if (!existingResult) {
			...
			// Randomly shuffle to create a result
			for (let i = driverIds.length - 1; i > 0; i--) {
				const j = Math.floor(Math.random() * (i + 1));
				[driverIds[i], driverIds[j]] = [driverIds[j], driverIds[i]];
			}
			...
			console.log(`  ✅ Result simulated for ${firstRace.name}`);			// Calculate points for predictions
			...
			for (const prediction of predictions) {
				...
				console.log(
					`  ${prediction.user.name} got ${totalPoints} points for ${firstRace.name}`
				);
			}
		} else {
